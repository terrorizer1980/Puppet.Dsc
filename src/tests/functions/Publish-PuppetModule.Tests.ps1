  ...
  InModuleScope puppet.dsc {
    ...
    Context 'When the forge token is not passed' {
      ...
      It 'Rethrows the exception' {
        { Publish-PuppetModule -PuppetModuleFolderPath 'TestDrive:\Foo' -ForgeUploadUrl 'https://localhost' -Publish 'true' -Build 'false'} | Should -Throw 'foo'
      }
    }
    ...
    Context 'When the PDK command is unsuccessful' {
      ...
      It 'Rethrows the exception' {
        { Publish-PuppetModule -PuppetModuleFolderPath 'TestDrive:\' -ForgeToken  'abcdefghijk' -ForgeUploadUrl 'https://localhost' -Publish 'true' -Build 'false'} | Should -Throw 'foo'
      }
    }
  }
}
